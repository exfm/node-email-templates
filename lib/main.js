...
var EmailTemplate = function(templateDirectory, done) {
  ...
  this.render = function(locals, html, text, stylesheet, callback) {
    ...
    function compileTemplate(template,data){
      var tmp = "";
          tmp = hbs.compile(template);
      return tmp(data);
    }
    
    html = compileTemplate(html, locals);
    text = (text) ? compileTemplate(text, locals) : '';
  ...
  fs.stat(templateDirectory, function(err, stats) {
    ...
    if (!stats.isDirectory())
      ...
      fs.stat(templatePath, function(err, stats) {
        ...
        that.html = path.join(templatePath, 'html.hbs');
        ...
        fs.stat(that.html, function(err, stats) {
          ...
          fs.readFile(that.html, 'utf8', function(err, data) {
            ...
            that.text = path.join(templatePath, 'text.hbs');
